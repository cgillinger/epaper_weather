...
class EPaperWeatherApp:
    ...
    def render_weather_layout(self):
            ...
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    if pressure_source == 'netatmo':
                        self.draw.text((x + 20, y + 155), "(Netatmo)", font=self.fonts['tiny'], fill=0)
                    elif pressure_source == 'smhi':
                        self.draw.text((x + 20, y + 155), "(SMHI)", font=self.fonts['tiny'], fill=0)
        ...
        # NYTT: Logga trycktrend-information
        pressure_trend = weather_data.get('pressure_trend', {})
        if pressure_trend and pressure_trend.get('trend') != 'unknown':
