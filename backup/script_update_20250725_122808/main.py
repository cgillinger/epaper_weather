...
class EPaperWeatherApp:
        ...
        for module_name, module_config in modules.items():
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    # Klockikon - FÖRBÄTTRAD: wi-time-7 + bättre position
                    clock_icon = self.icon_manager.get_system_icon('clock7', size=(32, 32))
                    if clock_icon:
                        self.paste_icon_on_canvas(clock_icon, x + 15, y + 25)
                        self.draw.text((x + 50, y + 30), now.strftime('%H:%M'), font=self.fonts['small_main'], fill=0)
                    else:
                        self.draw.text((x + 15, y + 30), now.strftime('%H:%M'), font=self.fonts['small_main'], fill=0)
                    
                    # Svenska datum
                    swedish_date = self.get_swedish_date(now)
                    self.draw.text((x + 15, y + 70), swedish_date, font=self.fonts['small_desc'], fill=0)
    ...
    def run_weather_app(self):
        ...
        try:
            ...
            print("\n" + "="*60)
            print("✅ E-PAPER VÄDERAPP MED HÖGUPPLÖSTA SVG-IKONER!")
            ...
            print("🚀 UPDATED: Högupplösta ikoner eliminerar pixling")
            print("🔧 Nya ikon-storlekar:")
            print("  🌤️ HERO väderikon: 96×96 (SVG-kvalitet)")
            print("  ↗️ Trycktrend-pil: 64×64 (SVG-kvalitet)")
            print("  📊 Barometer: 80×80 (SVG-kvalitet)")
            print("  🌦️ Prognos: 80×80 (SVG-kvalitet)")
            print("  ☀️ Sol-ikoner: 56×56 (SVG-kvalitet)")
            print("="*60)
