        ...
        for module_name, module_config in modules.items():
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    # Barometer-ikon (visar att det är lufttryck - nu med korrekt ikon!)
                    ...
                    # Trycktrend (bara text, ingen pil - som du ville)
                    if pressure > 1020:
                        trend_text = "Högtryck"
                    elif pressure < 1000:
                        trend_text = "Lågtryck"
                    ...
                    # Temperatur (på samma x-koordinat som "Imorgon" för alignment)
                    self.draw.text((x + 20, y + 80), f"{tomorrow_temp:.1f}°", font=self.fonts['medium_main'], fill=0)
                    
                    # Väderbeskrivning (på samma x-koordinat för perfekt alignment)
                    # ENBART en gång, inte dubbelt!
                    ...
                    # Imorgon väderikon (placerad så den inte krockar med text)
                    tomorrow_icon = self.icon_manager.get_weather_icon(tomorrow_symbol, is_night=False, size=(32, 32))
                    if tomorrow_icon:
                        # Placera ikonen längst upp till höger
                        self.paste_icon_on_canvas(tomorrow_icon, x + 180, y + 30)
                
                    # Ta bort eventuell extra textrendering som orsakar dubblering
                    # Bara dessa tre textrader ska visas: "Imorgon", temperatur, beskrivning
                    ...
                    # Klockikon (nu med korrekt ikon som existerar!)
                    clock_icon = self.icon_manager.get_system_icon('clock3', size=(24, 24))
                    if clock_icon:
                        self.paste_icon_on_canvas(clock_icon, x + 10, y + 18)
                        self.draw.text((x + 30, y + 20), now.strftime('%H:%M'), font=self.fonts['small_main'], fill=0)
                    else:
