...
class WeatherClient:
    """Klient för att hämta väderdata från SMHI, Netatmo och exakta soltider + CYKEL-VÄDER + SÄKER TEST-DATA"""
    ...
    def _apply_test_overrides(self, combined_data: Dict, test_data: Dict) -> Dict:
        """
        Applicera test-data på kombinerad väderdata
        """
        # Override nederbörd-data
        if 'precipitation' in test_data:
            combined_data['precipitation'] = test_data['precipitation']
            self.logger.info(f"🧪 Override precipitation: {test_data['precipitation']}mm/h")
        ...
        if 'forecast_precipitation_2h' in test_data:
            ...
            self.logger.info(f"🧪 Override forecast: {test_data['forecast_precipitation_2h']}mm/h")
    ...
    def combine_weather_data(self, smhi_data: Dict, netatmo_data: Dict, sun_data: Dict) -> Dict[str, Any]:
        ...
        TILLAGD: Säker test-data override för development
...
def test_weather_client():
    ...
    print("🧪 Test av Netatmo-integrerad WeatherClient MED CYKEL-VÄDER + TEST-DATA:")
