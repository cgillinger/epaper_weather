...
UPDATED: Förbättrade ikon-storlekar för bättre synlighet
"""
...
class EPaperWeatherApp:
        ...
        for module_name, module_config in modules.items():
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    # VÄDERIKON med exakt dag/natt-logik - STÖRRE STORLEK (120x120)
                    weather_icon = self.icon_manager.get_weather_icon_for_time(
                        smhi_symbol, current_time, sun_data, size=(120, 120)
                    )
                    if weather_icon:
                        # Placera ikon till höger om temperaturen - justerad position för större ikon
                        icon_x = x + 300
                        icon_y = y + 45
                    ...
                    # EXAKTA SOL-IKONER + tider - PERFEKT STORLEK (40x40)
                    ...
                    # Barometer-ikon - STÖRRE STORLEK (48x48)
                    barometer_icon = self.icon_manager.get_system_icon('barometer', size=(48, 48))
                    if barometer_icon:
                        self.paste_icon_on_canvas(barometer_icon, x + 20, y + 25)
                        # Tryck-värde bredvid ikon (FIXAD position - inte längre överlappande med pil)
                        self.draw.text((x + 75, y + 40), f"{int(pressure)}", font=self.fonts['medium_main'], fill=0)
                    else:
                        # Fallback utan ikon
                    ...
                    self.draw.text((x + 75, y + 100), "hPa", font=self.fonts['medium_desc'], fill=0)
                    ...
                    # TREND-PIL från Weather Icons - STÖRRE STORLEK (72x72)
                    trend_icon = self.icon_manager.get_pressure_icon(trend_arrow, size=(72, 72))
                    if trend_icon:
                        # Höger sida av modulen, men högre upp för att undvika avklippning
                        trend_x = x + width - 80  # 80px från höger kant för större ikon
                        trend_y = y + 90  # Högre upp för att ge utrymme för hela pilen (72px hög)
                    ...
                    # Imorgon väderikon - STÖRRE STORLEK (56x56)
                    tomorrow_icon = self.icon_manager.get_weather_icon(tomorrow_symbol, is_night=False, size=(56, 56))
                    if tomorrow_icon:
                        self.paste_icon_on_canvas(tomorrow_icon, x + 160, y + 25)
    ...
    def run_weather_app(self):
        ...
        try:
            ...
            print("🔧 UPDATED: Förbättrade ikon-storlekar för bättre synlighet")
            print("="*60)
