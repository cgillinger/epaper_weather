...
class EPaperWeatherApp:
        ...
        for module_name, module_config in modules.items():
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    # TREND-PIL från Weather Icons - VERKLIG HÖGUPPLÖST STORLEK (96x96)
                    trend_icon = self.icon_manager.get_pressure_icon(trend_arrow, size=(96, 96))
                    if trend_icon:
                        # Höger sida av modulen, optimerad position för 96x96
                        trend_x = x + width - 105  # 105px från höger kant för 96px ikon
                        trend_y = y + 80  # Centrerad vertikalt
                        self.paste_icon_on_canvas(trend_icon, trend_x, trend_y)
                        self.logger.info(f"↗️ Trycktrend-pil: 96x96 SVG-baserad ({trend_arrow})")
    ...
    def run_weather_app(self):
        ...
        try:
            ...
            print("  ↗️ Trycktrend-pil: 96×96 (SVG-kvalitet)")
