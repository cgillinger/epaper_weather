        ...
        for module_name, module_config in modules.items():
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    trend_icon = self.icon_manager.get_pressure_icon(trend_arrow, size=(48, 48))  # STÖRRE pil med ring
                    if trend_icon:
                        # Nedre högra hörnet av modulen (240x200px)
                        trend_x = x + width - 60  # 60px från höger kant (för större pil)
                        trend_y = y + height - 60  # 60px från botten (för större pil)
