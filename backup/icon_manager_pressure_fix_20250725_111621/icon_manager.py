...
FIXED: Barometer-ikon mapping + rätta pressure-pilar med ringar (towards-X-deg)
"""
...
class WeatherIconManager:
    ...
    def __init__(self, icon_base_path="icons/"):
        ...
        # QUICK FIX: Trycktrend-ikoner - använd befintliga pilar (wi-direction-X)
        self.pressure_mapping = {
            'rising': 'wi-direction-up',      # ↑ (stigande tryck) - FINNS
            'falling': 'wi-direction-down',   # ↓ (fallande tryck) - FINNS  
            'stable': 'wi-direction-right'    # → (stabilt tryck) - FINNS
        }
        ...
        print(f"✅ QUICK FIX: Använder befintliga pressure-ikoner (wi-direction-X)!")
    ...
    def get_pressure_icon(self, trend, size=(48, 48)):
        """
        Hämta trycktrend-ikon (QUICK FIX: Använder befintliga wi-direction-X ikoner)
        
        Args:
            trend: 'rising', 'falling', eller 'stable'
            size: Tuple med ikon-storlek (default: 48x48 för stora trend-pilar)
            
        Returns:
            PIL Image-objekt eller None vid fel
        """
        icon_name = self.pressure_mapping.get(trend, 'wi-direction-right')  # Fallback till stabilt
        self.logger.info(f"🎯 Pressure icon mapping: {trend} → {icon_name}")
    ...
    def optimize_for_epaper(self, image):
        ...
        FÖRBÄTTRAD för skarpare ikoner på E-Paper
        ...
        try:
            ...
            # FÖRBÄTTRAD E-Paper optimering för skarpare ikoner
            # Höj kontrast rejält för E-Paper
            enhancer = ImageEnhance.Contrast(image)
            image = enhancer.enhance(2.5)  # Ökat från 1.8 till 2.5
            
            # Förbättra skärpa
            sharpness_enhancer = ImageEnhance.Sharpness(image)
            image = sharpness_enhancer.enhance(1.5)
            
            # Justera ljusstyrka för bättre threshold
    ...
    def test_icon_loading(self):
        ...
        # Test tryckikon (QUICK FIX: Befintliga wi-direction-X ikoner)
        try:
            pressure_icon = self.get_pressure_icon('rising', size=(48, 48))
...
def test_weather_icon_manager():
    ...
    print("🧪 Testar WeatherIconManager med FIXED pressure icons...")
    ...
    print(f"\n💡 QUICK FIX: Pressure-ikoner använder befintliga wi-direction-X filer!")
