        ...
        for module_name, module_config in modules.items():
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    # hPa-text (flyttad upp för bättre layout)
                    self.draw.text((x + 60, y + 80), "hPa", font=self.fonts['medium_desc'], fill=0)
                    
                    # RIKTIGA TREND-TEXT (från 3h-analys) - RADBRYTS OM DET ÄR "Samlar data"
                    if trend_text == 'Samlar data':
                        self.draw.text((x + 20, y + 110), "Samlar", font=self.fonts['medium_desc'], fill=0)
                        self.draw.text((x + 20, y + 135), "data", font=self.fonts['medium_desc'], fill=0)
                    else:
                        self.draw.text((x + 20, y + 110), trend_text, font=self.fonts['medium_desc'], fill=0)
                    ...
                    if pressure_trend.get('change_3h') is not None and pressure_trend.get('trend') != 'insufficient_data':
                        ...
                        change_y = y + 160 if trend_text == 'Samlar data' else y + 135
                        self.draw.text((x + 20, change_y), change_text, font=self.fonts['small_desc'], fill=0)
                    
                    # TREND-PIL från Weather Icons - FLYTTAD TILL NEDRE HÖGRA HÖRNET
                    trend_icon = self.icon_manager.get_pressure_icon(trend_arrow, size=(32, 32))  # Större pil
                    if trend_icon:
                        # Nedre högra hörnet av modulen (240x200px)
                        trend_x = x + width - 50  # 50px från höger kant
                        trend_y = y + height - 50  # 50px från botten
                    ...
                    if pressure_source == 'netatmo':
                        self.draw.text((x + 20, y + height - 25), "(Netatmo)", font=self.fonts['tiny'], fill=0)
                    elif pressure_source == 'smhi':
                        self.draw.text((x + 20, y + height - 25), "(SMHI)", font=self.fonts['tiny'], fill=0)
