        ...
        for module_name, module_config in modules.items():
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    # TREND-PIL från riktiga data
                    trend_icon = self.icon_manager.get_pressure_icon(trend_arrow, size=(24, 24))
                    if trend_icon:
                        # Placera trend-pil bredvid tryck-värdet
                        trend_x = x + 150 if barometer_icon else x + 120
                        self.paste_icon_on_canvas(trend_icon, trend_x, y + 40)
                    
                    # hPa-text
                    self.draw.text((x + 60, y + 80), "hPa", font=self.fonts['medium_desc'], fill=0)
                    
                    # RIKTIGA TREND-TEXT (från 3h-analys)
                    ...
                    if pressure_trend.get('change_3h') is not None and pressure_trend.get('trend') != 'insufficient_data':
                        ...
                        self.draw.text((x + 20, y + 135), change_text, font=self.fonts['small_desc'], fill=0)
                    
                    # NYTT: Visa tryck-källa (diskret)
                    if pressure_source == 'netatmo':
                        self.draw.text((x + 20, y + 160), "(Netatmo)", font=self.fonts['tiny'], fill=0)
                    elif pressure_source == 'smhi':
                        self.draw.text((x + 20, y + 160), "(SMHI)", font=self.fonts['tiny'], fill=0)
