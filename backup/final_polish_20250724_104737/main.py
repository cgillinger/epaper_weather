            ...
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    if weather_icon:
                        ...
                        icon_x = x + 300  # Mer space från temperatur-text
                        icon_y = y + 60   # Samma höjd som temperatur
                    ...
                    # SOL-IKONER + tider (förbättrad positionering)
                    sunrise_str = sunrise.strftime('%H:%M')
                    sunset_str = sunset.strftime('%H:%M')
                    
                    # Soluppgång - ikon + tid
                    sunrise_icon = self.icon_manager.get_sun_icon('sunrise', size=(32, 32))
                    if sunrise_icon:
                        self.paste_icon_on_canvas(sunrise_icon, x + 20, y + 215)
                        self.draw.text((x + 55, y + 224), sunrise_str, font=self.fonts['medium_desc'], fill=0)
                    else:
                        # Fallback utan ikon
                        self.draw.text((x + 20, y + 220), f"🌅 {sunrise_str}", font=self.fonts['medium_desc'], fill=0)
                    
                    # Solnedgång - ikon + tid
                    sunset_icon = self.icon_manager.get_sun_icon('sunset', size=(32, 32))
                    if sunset_icon:
                        self.paste_icon_on_canvas(sunset_icon, x + 150, y + 215)
                        self.draw.text((x + 185, y + 224), sunset_str, font=self.fonts['medium_desc'], fill=0)
                    else:
                        # Fallback utan ikon
                    ...
                    # Trycktrend med ikon (förbättrat)
                    if pressure > 1020:
                        trend = "rising"
                        trend_text = "Högtryck"
                    elif pressure < 1000:
                    ...
                    # Trycktrend-ikon (större storlek)
                    pressure_icon = self.icon_manager.get_pressure_icon(trend, size=(24, 24))
                    if pressure_icon:
                        self.paste_icon_on_canvas(pressure_icon, x + 170, y + 130)
