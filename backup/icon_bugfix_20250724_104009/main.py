...
class EPaperWeatherApp:
    ...
    def render_weather_layout(self):
        ...
        for module_name, module_config in modules.items():
            if module_config['enabled']:
                ...
                if module_name == 'main_weather':
                    ...
                    # VÄDERIKON (stor) - dag/natt-variant
                    weather_icon = self.icon_manager.get_weather_icon_for_time(
                        smhi_symbol, current_time, sun_data, size=(64, 64)
                    )
                    if weather_icon:
                        # Placera ikon bredvid temperaturen
                        self.paste_icon_on_canvas(weather_icon, x + 240, y + 50)
                    ...
                    # Soluppgång - ikon + tid
                    sunrise_icon = self.icon_manager.get_sun_icon('sunrise', size=(24, 24))
                    if sunrise_icon:
                        self.paste_icon_on_canvas(sunrise_icon, x + 20, y + 220)
                        self.draw.text((x + 50, y + 224), sunrise_str, font=self.fonts['medium_desc'], fill=0)
                    else:
                        # Fallback utan ikon
                        self.draw.text((x + 20, y + 220), f"🌅 {sunrise_str}", font=self.fonts['medium_desc'], fill=0)
                    
                    # Solnedgång - ikon + tid
                    sunset_icon = self.icon_manager.get_sun_icon('sunset', size=(24, 24))
                    if sunset_icon:
                        self.paste_icon_on_canvas(sunset_icon, x + 150, y + 220)
                        self.draw.text((x + 180, y + 224), sunset_str, font=self.fonts['medium_desc'], fill=0)
                    else:
                        # Fallback utan ikon
                        self.draw.text((x + 150, y + 220), f"🌇 {sunset_str}", font=self.fonts['medium_desc'], fill=0)
                    ...
                    # Trycktrend-ikon
                    pressure_icon = self.icon_manager.get_pressure_icon(trend, size=(20, 20))
                    if pressure_icon:
                        self.paste_icon_on_canvas(pressure_icon, x + 140, y + 130)
        ...
        self.logger.info("🎨 Layout renderad med Weather Icons (fallback-ikoner)")
        
        # Visa cache-statistik
        cache_stats = self.icon_manager.get_cache_stats()
        self.logger.info(f"💾 Ikon-cache: {cache_stats['total_cached_icons']} ikoner")
